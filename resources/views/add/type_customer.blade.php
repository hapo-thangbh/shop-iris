...
@section('content')
    <div class="container-fluid">
        <div class="row">
            <form class="w-100 my-3" action="{{ route('add.store_type_customer') }}" method="POST">
                @csrf
                <input name="name" required class="form-control col-10 col-md-3 my-1" placeholder="Loại khách hàng">
                ...
                @foreach($types as $type)
                    <tr>
                        ...
                        <td class="text-center">
                            <form action="{{ route('setting.delete_customer', $type->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
